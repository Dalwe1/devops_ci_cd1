				...
				steps {
					...
					checkout(
						[
							...
							userRemoteConfigs: [[url: 'https://github.com/Dalwe1/devops_ci_cd1.git.git']]						
						]
						)
	...
	stage('Deploying on Docker') {
		...
		steps {
			script {
				if (env.BRANCH_NAME == 'master'){
				sh """
				if [ `curl -o /dev/null -s -w "%{http_code}\n" http://65.0.89.203:83/index.html` = 200 ];
				then
				...
				sh """
				if [ `curl -o /dev/null -s -w "%{http_code}\n" http://65.0.89.203:82/index.html` = 200 ];
				then
				echo "Merge Successful"
				else
				echo "Merge Failed"
	...
	post {
		success {
			...
			if (env.BRANCH_NAME == 'master'){
				...
				sh('git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/Dalwe1/devops_ci_cd1.git HEAD:master')
					}
				} else if (env.BRANCH_NAME == 'dev'){
